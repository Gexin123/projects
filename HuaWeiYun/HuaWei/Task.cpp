...
void FirstLevel(TaskStack& ts) {
	...
	//ts._level = mBenchType2TaskLevel[vBenchInfo[maxBenchNum].getBenchType()];
	//ts._level = mBenchType2TaskLevel[vBenchInfo[maxBenchNum].getBenchType()];
	...
	if (currBench.getBenchType() > 2 && currBench.getBenchType() < 7) {
		...
		if (sSevenLackRaw.size() != 0) {
			...
			if (cnt) {
				if (CurrentMapNum == 4) {
					//fprintf(stderr, "7缺了一个原材料是%d\n", currBench.getBenchType());
					ts._level = TaskLevel::LEVEL8;
				}
				else {
					ts._level = TaskLevel::LEVEL4;
				}
				//qTaskPool.push(ts);
		...
		if (CurrentMapNum == 4 && currBench.getBenchType() == 3) {
			// 地图4将4号工作台优先级提高，比56优先级更高
			ts._level = TaskLevel::LEVEL5;
			//qTaskPool.push(ts);
			//fprintf(stderr, "seven lack %d a raw material!\n", currBench.getBenchType());
			return;
		}
		...
		if (CurrentMapNum == 1) {
			// 地图1，前8000帧123456卖给9的优先级最低
			int lastNum = ts._qTaskList[totalBench.size() - 2]._qBench;
			if (vBenchInfo[lastNum].getBenchType() == 6) {
				ts._level = TaskLevel::LEVEL9;
			}
			else {
...
bool TaskPool::operator()(TaskStack tq1, TaskStack tq2) {
	...
	//// 同层优先级按长度排序
	//if (tq1._qTaskList.size() != tq2._qTaskList.size()) {
	//	return tq1._qTaskList.size() < tq2._qTaskList.size();
	//}
